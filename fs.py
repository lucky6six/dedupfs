...
def compute_difference_detla(chunk1, chunk2):
    ...
    diff_size = int((1-diff.ratio()) * size_per_chunk)
     # 获取当前时间
    end_time = time.time()
    # 计算时间差
...
def simulate_deduplication(directories, chunk_size, type_of_fs):
    """
...
def print_info(elapsed_time, time_diff, total_bytes, unique_bytes, modified_bytes, dedup_rate):
...
def main():
    ...
    start_time = time.time()
    # 调用去重模拟
    total_bytes, unique_bytes, modified_bytes, dedup_rate = simulate_deduplication(directories,size_per_chunk,type_of_fs)
    # 获取当前时间
    end_time = time.time()
    elapsed_time = end_time - start_time
