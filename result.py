...
def extract_metrics_from_file(file_path):
    ...
    feature_time_pattern = r"特征提取时间:\s*(\d+\.\d+)\s*秒"
    time_pattern = r"代码执行时间:\s*(\d+\.\d+)\s*秒"
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        ...
        feature_time_match = re.search(feature_time_pattern, content)
        ...
        if feature_time_match:
            metrics["Feature Time (s)"] = float(feature_time_match.group(1))
        if time:
            metrics["Time (s)"] = float(time.group(1))
        ...
        for file_type in file_types:
            ...
            if os.path.exists(file_path):
                ...
                all_metrics.append({
                    ...
                    "feature Time (s)": metrics["Feature Time (s)"],
...
def print_metrics_summary(metrics):
    ...
    """
    print(f"{'Directory':<25}{'Block Size (KB)':<20}{'File Type':<10}{'Deduplication Rate':<20}{'Throughput (MB/s)':<20}{'Computation Time (s)':<20}{'feature Time (s)':<20}{'Time (s)':<20}")
    print("="*150)
    
    for metric in metrics:
        print(f"{metric['Directory']:<25}{metric['Block Size (KB)']:<20}{metric['File Type']:<10}{metric['Deduplication Rate']:<20}{metric['Throughput (MB/s)']:<20}{metric['Computation Time (s)']:<20}{metric['feature Time (s)']:<20}{metric['Time (s)']:<20}")
